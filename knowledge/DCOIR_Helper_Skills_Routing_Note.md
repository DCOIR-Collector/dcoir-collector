...
- dcoir-memory-preflight
  ...
  - Use again after a blocker or failed attempt is successfully overcome when the lesson may be reusable across similar work.
  - Treat reusable blocker recovery as a promotion-candidate staging step, not automatic canonical memory mutation.
...
- dcoir-session-tracker
  - Use for session-local notes, unfinished items, exportable handoff artifacts, and future session-local buffer participation where appropriate.
- dcoir-plan-tracker
  - Use for deep task decomposition, progress tracking, resume state persistence, blocker capture, and milestone-visible execution flow.

### Documentation, prompt, and design work
- dcoir-readme-maintainer
  - Use to maintain root and folder README surfaces, README navigation links, and missing README coverage in the DCOIR project.
  - Do not use it for broad knowledge-doc generation or authority decisions.
...
- Improve the README, update folder README, or fix README navigation.
  - Route to dcoir-readme-maintainer.
