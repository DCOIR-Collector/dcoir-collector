...
## Task-time schema gate addendum
Run a compact schema gate before Airtable writes, deletes, cleanup, merge/dedupe, migration, Delete Queue work, schema-sensitive filters/sorts, select-option use, linked-record dependency checks, formula/id generation, or code that depends on Airtable fields. A fresh cache may support read-only lookup, but live schema readback is mandatory before destructive or schema-changing work.
