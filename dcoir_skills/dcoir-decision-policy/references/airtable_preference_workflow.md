<!-- skill-marker: updated-skill|20260425T071800Z|T2.3-airtable-first-skill-repair|source-update|dcoir-decision-policy|airtable_preference_workflow.md -->

# Airtable Preference Workflow

## Purpose
Use Airtable as the durable working-state surface for operator preferences and decision overlays that should survive across DCOIR sessions before or alongside later Airtable promotion.

Airtable remains the readable promoted surface for durable project-facing rules that should ship with the skill or helper memory.
...
- `references/decision_learning_log.json` remains the situational or pending-learning Airtable surface when a rule should stay reviewable before being treated as approved durable policy.
- Airtable table `dcoir-decision-policy` is the helper-readable continuity surface for snapshots, delivery-friction notes, campaign posture, and promotion context.
...
4. If Airtable and approved Airtable durable overlays conflict, surface the conflict and ask whether to replace, narrow, or keep both with scoped conditions.
...
4. Fill source fields when known so later Airtable promotion has provenance.
5. Update `last_confirmed_text` whenever the preference is re-stated, materially refined, or reused successfully.

## Promotion workflow
At the next safe Airtable flush point, evaluate Airtable-held implemented preferences for repo promotion.

Preferred Airtable promotion targets:
...
- helper-memory continuity, delivery posture, or campaign notes -> Airtable table `dcoir-decision-policy`

Do not treat Airtable as control-plane authority.
Do not silently rewrite approved Airtable durable rules when the Airtable entry conflicts with the current matrix.
...
- `general chat`: preserve in Airtable for continuity, but do not automatically elevate it into project-governed Airtable files unless it materially affects DCOIR behavior too.

## Recommended sync behavior
- Keep Airtable as the faster durable capture surface.
- Keep Airtable as the promoted readable policy surface.
- After Airtable promotion, keep the Airtable row and record the promotion note in `notes` instead of deleting the row.
