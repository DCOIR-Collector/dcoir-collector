#!/usr/bin/env python3
"""Render a DCOIR live-test-remediation memory markdown file from JSON state."""
...
def fmt_item(item: dict[str, Any]) -> str:
    ...
    parts = [f"- **{title}** (status: {status}; priority: {priority})"]
...
def build_markdown(state: dict[str, Any]) -> str:
    ...
    lines = ["---", "artifact_type: dcoir-live-test-remediation-memory", "schema_version: 1", f"project: {state.get('project', 'AFRICOM_SOC_IR / DCOIR')}", f"exported_at_utc: {exported_at}", "authority_basis:"]
    lines.extend([f"  - {item}" for item in authority_basis] or ["  - none-recorded"])
    lines.extend(["---", "", "# DCOIR Live Test Remediation Memory", "", "## Current focus", state.get("current_focus", "not specified"), "", "## Active remediation queue"])
    lines.extend(section(state.get("active_remediation_queue", [])))
    lines.extend(["", "## Recurring findings"])
    lines.extend(bullets(state.get("recurring_findings", [])))
    lines.extend(["", "## Deep-regression watchlist"])
    lines.extend(bullets(state.get("deep_regression_watchlist", [])))
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: python render_live_test_remediation_memory.py state.json output.md")
        return 1
