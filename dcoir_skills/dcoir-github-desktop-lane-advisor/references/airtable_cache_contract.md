...
- `Local Configuration Registry` (`tblcJxCoYGpEda0FM`): read live only when launcher commands need config-name safety guidance or when installed-skill/admin state matters.
- `Admin Registry` (`tblFaJW1V2DPc9css`): read live only when launcher commands need config-name safety guidance or when installed-skill/admin state matters.
