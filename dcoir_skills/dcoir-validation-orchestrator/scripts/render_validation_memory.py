...
def fmt_item(item: dict[str, Any]) -> str:
    ...
    for key in ['scope', 'campaign_scope', 'why', 'buffer_state', 'flush_trigger', 'next_action']:
        value = item.get(key, '')
        if value:
...
def build_markdown(state: dict[str, Any]) -> str:
    ...
    lines.extend(['', '## Buffered validation state'])
    lines.extend(bullets(state.get('buffered_state', [])))
    lines.extend(['', '## Deferred review counters'])
    lines.extend(bullets(state.get('deferred_review_counters', [])))
