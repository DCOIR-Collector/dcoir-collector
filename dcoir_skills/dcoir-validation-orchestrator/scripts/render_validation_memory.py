#!/usr/bin/env python3
"""Render a DCOIR validation-orchestrator memory markdown file from JSON state."""
...
def fmt_item(item: dict[str, Any]) -> str:
    ...
    for key in ["scope", "why", "next_action"]:
        value = item.get(key, "")
        if value:
...
def build_markdown(state: dict[str, Any]) -> str:
    ...
    lines = ["---", "artifact_type: dcoir-validation-orchestrator-memory", "schema_version: 1", f"project: {state.get('project', 'AFRICOM_SOC_IR / DCOIR')}", f"exported_at_utc: {exported_at}", "authority_basis:"]
    ...
    lines.extend(["", "## Reusable gates and thresholds"])
    lines.extend(bullets(state.get("reusable_gates", [])))
    lines.extend(["", "## Open evidence requirements"])
    lines.extend(bullets(state.get("open_evidence_requirements", [])))
...
def main() -> int:
    if len(sys.argv) != 3:
        print("Usage: python render_validation_memory.py state.json output.md")
        return 1
