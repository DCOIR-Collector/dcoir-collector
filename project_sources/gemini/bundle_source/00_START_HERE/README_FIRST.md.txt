...
The build wrapper reads the maintained `knowledge/*.md` files directly and packages only the manifest-listed `knowledge_attachment_sources` as `02_PRIME_AGENT_ATTACHMENTS/*.md.txt` inside the generated release ZIP. The maintained knowledge files are the only editable source of truth for shared content in this major-version build. Maintained knowledge pages can remain operator-only when the manifest classifies them outside the runtime attachment set.

## Attachment-inventory rule

The attachment map, the manifest `knowledge_attachment_sources` inventory, and the bundle-facing maintained `knowledge/*.md` set must stay aligned. If the shared attachment set changes, update those surfaces in the same bounded change set so manual build, validate-on-push, and compile all continue to agree about what the bundle is supposed to ship. If a maintained knowledge page is meant to guide future Gemini source work without shipping to runtime, keep it out of the attachment map and record it under `operator_only_knowledge_sources` in the manifest.
