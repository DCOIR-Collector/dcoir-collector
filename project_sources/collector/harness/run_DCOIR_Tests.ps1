...
function Add-CollectorCapabilityCoverageRows {
  ...
  $rows = @(
    ...
    @{ Id='collector.enrich.session_lifecycle.basic'; Cat='Enrich sessions'; Claim='Enrich start/add/finalize preserves session identity and finalized output contract.'; Sources=@('project_sources/collector/source/DCOIR_Collector.ps1'); Class='covered_end_to_end_by_fullregression'; Suites=@('SessionBehavior'); Steps=@('52_EnrichStartTcp','53_EnrichAddLogTextSecurity','ZZ_SessionReuseValidation','56_EnrichFinalizeRequestedOriginal','ZZ_RequestedFinalizeOutputContract'); Risk='high'; Value='high'; Assertions=@('session starts','add reuses session identity','requested finalize emits expected finalized contract for the original session') },
