...
function Add-CollectorCapabilityCoverageRows {
  ...
  $rows = @(
    ...
    @{ Id='collector.enrich.session_lifecycle.edge_cases'; Cat='Enrich sessions'; Claim='Stale/finalized, missing, explicit, force-new, add-after-finalize, and cleanup-after-open-session behavior is validated.'; Sources=@('project_sources/collector/source/DCOIR_Collector.ps1'); Class='covered_end_to_end_by_fullregression'; Suites=@('SessionBehavior'); Steps=@('54_EnrichAddExplicitSession','ZZ_ExplicitSessionReuseValidation','55_EnrichForceNewSession','ZZ_ForceNewSessionValidation','56_EnrichFinalizeRequestedOriginal','ZZ_RequestedFinalizeOutputContract','57_EnrichAddFinalizedRequestedSession','58_EnrichFinalizeOpen','ZZ_OpenFinalizeOutputContract','59_EnrichAddAfterFinalize','ZZ_AddAfterFinalizeValidation','60_MissingRequestedEnrichSession','61_CleanupAfterOpenSession'); Risk='medium'; Value='high'; Assertions=@('explicit requested session is reused','force-new creates a different session','requested finalized session emits finalized contract','finalized requested session is rejected','open finalized session emits finalized contract','add-after-finalize creates a new session','missing requested session errors','cleanup succeeds while a session is open') },
