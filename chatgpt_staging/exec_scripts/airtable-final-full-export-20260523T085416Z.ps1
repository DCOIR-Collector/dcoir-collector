...
$requestId = 'airtable-final-full-export-20260523T085416Z'
$outputPrefix = 'dcoir_airtable_final_full_export'
...
$readbackDir = Join-Path $reportDir 'airtable_full_export_readback'
...
foreach ($relative in @('diagnostic_index.md','command_context.json','metadata\metadata_coverage.json','schema\schema.summary.json','schema\schema.base_tables.json','terminal_transcript.txt','run.log.txt')) {
...
$schemaSourceDir = Join-Path $runFolder.FullName 'schema'
if (Test-Path -LiteralPath $schemaSourceDir -PathType Container) {
    $schemaReadbackDir = Join-Path $readbackDir 'schema'
    New-Item -ItemType Directory -Force -Path $schemaReadbackDir | Out-Null
    Get-ChildItem -LiteralPath $schemaSourceDir -Filter '*.json' -File -ErrorAction Stop | Sort-Object Name | ForEach-Object {
        Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $schemaReadbackDir $_.Name) -Force
    }
}
...
$summary = [ordered]@{
    schema = 'dcoir.airtable.final_full_export.readback.v1'
    request_id = $requestId
    success = $true
    purpose = 'Read-only final full Airtable base export before Airtable removal from the agent.'
...
$summaryJsonPath = Join-Path $readbackDir 'airtable_final_full_export_summary.json'
$summaryMdPath = Join-Path $reportDir 'airtable_final_full_export_report.md'
$summary | ConvertTo-Json -Depth 12 | Out-File -FilePath $summaryJsonPath -Encoding utf8

$lines = New-Object System.Collections.Generic.List[string]
$lines.Add('# Airtable Final Full Export Report')
$lines.Add('')
