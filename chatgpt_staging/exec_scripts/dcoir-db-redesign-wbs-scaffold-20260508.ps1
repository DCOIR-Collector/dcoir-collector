...
foreach ($name in $required) { if (-not $fieldByName.ContainsKey($name)) { throw ('Missing WBS field: ' + $name) } }
...
foreach ($file in $inputFiles) {
  ...
  $rows = @(Get-Content -LiteralPath $file -Raw -Encoding UTF8 | ConvertFrom-Json)
  foreach ($row in $rows) { $targetRows.Add($row) | Out-Null }
}
...
try {
  ...
  [ordered]@{ result='success'; created_count=$created.Count; skipped_existing_count=$existingRows.Count; total_expected_keys=$targetKeys.Count; output_dir=$runRoot } | ConvertTo-Json -Depth 8
}
catch {
  [ordered]@{
    result = 'failed'
