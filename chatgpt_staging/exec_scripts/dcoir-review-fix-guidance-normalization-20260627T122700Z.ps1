...
function Restore-SourceFiles {
    & git restore --staged --worktree -- $sourcePaths 2>$null
    if ($LASTEXITCODE -ne 0) {
        Write-Warning ('Unable to restore source files after failure; git restore exit code {0}' -f $LASTEXITCODE)
    }
}
...
def replace_once_or_confirm(path: str, old: str, new: str) -> None:
    target = Path(path)
...
'@ | Set-Content -LiteralPath $patchPath -Encoding UTF8
    ...
    $smokePath = Join-Path $env:TEMP 'dcoir_review_fix_guidance_smoke.py'
@'
...
root = Path.cwd()
os.environ.setdefault("GITHUB_REPOSITORY", "DCOIR-Collector/dcoir-collector")
os.environ.setdefault("PR_NUMBER", "317")
os.environ.setdefault("OPENROUTER_API_KEY", "test-openrouter-key")
script = root / "scripts" / "openrouter_pr_review_pareto_context.py"
spec = importlib.util.spec_from_file_location("openrouter_pr_review_pareto_context_smoke", script)
assert spec is not None and spec.loader is not None
mod = importlib.util.module_from_spec(spec)
sys.modules[spec.name] = mod
spec.loader.exec_module(mod)
config = mod.load_pareto_context_config(str(root / ".github" / "openrouter-pr-review-pareto.yml"))
comment = mod.base.build_inline_comment(
    {
        "title": "Global state write",
        "severity": "high",
        "confidence": 0.95,
...
assert "```text\n# The function should record state through governed storage, not global scope.\n```" in comment
result = mod.harden_python_dynamic_exec_fix_result(
    {"suggested_replacement": "return eval(expression, {'__builtins__': {}})", "notes": "Restricted globals make this safe."},
    {"title": "Arbitrary Python code execution via eval on caller-controlled expression", "body": "eval runs caller-controlled Python code."},
    "project_sources/collector/tools/dcoir_review_intentional_python_probe.py",
    "    return eval(expression, {'__builtins__': __builtins__, 'os': os, 'Path': Path})",
)
assert result["suggested_replacement"] == ""
assert "eval(" not in result["replace"]
assert "exec(" not in result["replace"]
assert "ast.literal_eval" in result["replace"]
assert "Restricted globals make this safe" not in result["notes"]
'@ | Set-Content -LiteralPath $smokePath -Encoding UTF8
    ...
    Invoke-Native -FilePath 'python' -ArgumentList @($smokePath)
    ...
    Invoke-Native -FilePath 'git' -ArgumentList @('fetch', 'origin', 'main')
    Invoke-Native -FilePath 'git' -ArgumentList @('rebase', 'origin/main')
