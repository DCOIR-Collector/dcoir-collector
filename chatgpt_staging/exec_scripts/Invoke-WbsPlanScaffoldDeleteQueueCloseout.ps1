...
$RequestId = 'exec-20260508-cleanup-plan-scaffold-delete-queue-closeout-001'
...
function New-RunDir {
  ...
  $out = Join-Path $downloads ("cleanup_plan_scaffold_delete_queue_closeout_{0}" -f $stamp)
...
function Get-AllRecords { param([string]$TableId,[string]$FilterFormula=$null)
...
function Add-QueueRecords { param([object[]]$QueueRows)
...
try {
  ...
  $planRows = Get-AllRecords -TableId $planTable -FilterFormula ("{plan_id}='{0}'" -f $PlanKey.Replace("'","\'"))
  $wbsRows = Get-AllRecords -TableId $wbsTable -FilterFormula ("{plan_key}='{0}'" -f $PlanKey.Replace("'","\'"))
  $scaffoldRows = Get-AllRecords -TableId $scaffoldTable -FilterFormula ("{plan_key}='{0}'" -f $PlanKey.Replace("'","\'"))
  ...
  $queueRows = @($targets | ForEach-Object {
    @{
  ...
  Write-JsonFile -Path (Join-Path $OutDir 'target_records.json') -Object $targets
  ...
  $summary = [pscustomobject]@{
    ...
    targets = [pscustomobject]@{ plans=$planRows.Count; wbs=$wbsRows.Count; scaffold=$scaffoldRows.Count; total=$targets.Count }
    approved_to_delete = $true
    delete_stage = 'pending'
