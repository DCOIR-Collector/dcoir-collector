...
$RequestId = 'exec-20260508-cleanup-plan-scaffold-delete-queue-closeout-002'
...
function New-RunDir {
  ...
  $out = Join-Path $downloads ("cleanup_plan_scaffold_delete_queue_closeout_v002_{0}" -f $stamp)
...
try {
  ...
  foreach ($t in @($targets | Select-Object -First 20)) {
