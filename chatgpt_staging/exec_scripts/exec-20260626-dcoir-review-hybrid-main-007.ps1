...
$tempInnerScript = Join-Path $env:RUNNER_TEMP 'exec-20260626-dcoir-review-hybrid-main-007-inner.ps1'
$trackedPaths = @(
...
$summaryPath = 'chatgpt_staging/status_reports/chatgpt-exec/exec-20260626-dcoir-review-hybrid-main-007/diagnostic-summary.md'
...
$innerText = $innerText.Replace("git diff --check`nif (`$LASTEXITCODE -ne 0) { throw 'git diff --check failed' }", "git -c core.whitespace=blank-at-eol,blank-at-eof,space-before-tab,cr-at-eol diff --check -- scripts/openrouter_pr_review_pareto_context.py scripts/openrouter_pr_review_pareto_context_selftest.py .github/openrouter-pr-review-pareto.yml ops/requests/apply_patch/README.md`nif (`$LASTEXITCODE -ne 0) { throw 'git diff --check failed' }")
...
try {
  ...
  @(
    ...
    '',
    '- result: success',
    '- inner_script: exec-20260626-dcoir-review-hybrid-main-003.ps1 with unanchored reason wording preserved and source-scoped CR-aware diff check',
