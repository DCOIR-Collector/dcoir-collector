...
$requestId = 'exec-20260626-dcoir-review-fix-synthesis-003'
...
function Invoke-Python {
  ...
  $output = & $cmd @displayArgs 2>&1
  $code = $LASTEXITCODE
  Add-OutputLines -Output $output
  if ($code -ne 0) { throw "python failed with exit $code`: $($displayArgs -join ' ')" }
}

function Get-TrackedBlob {
  param([string]$Path)
  $line = & git ls-files -s -- $Path 2>&1
  $code = $LASTEXITCODE
  if ($code -ne 0) {
    Add-OutputLines -Output $line
    throw "Unable to read tracked blob for $Path"
  }
  $lineText = ($line | Select-Object -First 1 | Out-String).Trim()
...
'@
  ...
  Add-OutputLines -Output (& git diff --stat -- $runnerPath $selftestPath 2>&1)
  ...
  $script:BranchHead = (& git rev-parse HEAD).Trim()
