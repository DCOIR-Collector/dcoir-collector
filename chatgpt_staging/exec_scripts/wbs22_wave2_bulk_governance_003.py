...
API='https://api.airtable.com/v0'; BASE=os.environ.get('DCOIR_AIRTABLE_BASE_ID') or 'appM4KSwnVf3G3OTK'; REQ='exec-20260507-wbs22-wave2-bulk-governance-003'
...
REC={'wbs04':'recSJWdke5hsh9E4H','wbs05':'recPiAsI0H1F7JF2s','wbs06':'recxw7OJh28aLkG2y','plan':'recoLHyurY4OZx3K8'}
EKEY='VE-WBS22-WAVE2-BULK-EXEC-20260507-003'; CID='CHK-DCOIR-WBS22-WAVE2-BULK-EXEC-20260507-003'
...
def main():
 ...
 before={k:get(T['wbs'],REC[k]) for k in ['wbs04','wbs05','wbs06']}; before['plan']=get(T['plan'],REC['plan']); actions=[]
 if not find(T['ev'],'evidence_key',EKEY):
  create(T['ev'],[{'fields':{E['key']:EKEY,E['case']:'WBS22-WAVE2-BULK',E['work']:'CLEANUP-WBS-22-05',E['summary']:'Wave 2 governance consistency batch completed by chatgpt-exec. WBS22-04 and WBS22-05 complete, WBS22-06 active, plan pointer advanced.',E['source']:f'chatgpt_staging/status_reports/chatgpt-exec/{REQ}/workflow_report.md',E['created']:ts,E['updated']:ts,E['result']:'passed',E['retention']:'operational'}}]); actions.append('evidence')
 patch(T['wbs'],[
  {'id':REC['wbs04'],'fields':{W['state']:'complete',W['validation']:add(before['wbs04']['fields'].get('validation_notes',''),f'Wave 2 preparation completed by {REQ} at {ts}. Evidence {EKEY}.'),W['context']:add(before['wbs04']['fields'].get('context',''),f'Wave 2 preparation completed by {REQ}.')}},
  {'id':REC['wbs05'],'fields':{W['state']:'complete',W['validation']:add(before['wbs05']['fields'].get('validation_notes',''),f'Wave 2 execution completed by {REQ} at {ts}. Evidence {EKEY}.'),W['context']:add(before['wbs05']['fields'].get('context',''),f'Wave 2 execution completed by {REQ}.')}},
  {'id':REC['wbs06'],'fields':{W['state']:'active',W['validation']:add(before['wbs06']['fields'].get('validation_notes',''),f'Activated for Wave 3 by {REQ} at {ts}.'),W['context']:add(before['wbs06']['fields'].get('context',''),f'Activated for Wave 3 by {REQ}.')}}]); actions.append('wbs')
 patch(T['plan'],[{'id':REC['plan'],'fields':{P['active']:'CLEANUP-WBS-22-06',P['title']:'Prepare Wave 3 duplicate and merge candidates',P['next']:'Continue with Wave 3 using compatible bulk batches. Stop before Wave 6 review.',P['updated']:ts}}]); actions.append('plan')
 if not find(T['chk'],'checkpoint_id',CID):
  create(T['chk'],[{'fields':{C['id']:CID,C['session']:'DCOIR-WBS22-WAVE2-BULK',C['summary']:'Wave 2 batch complete. WBS22-06 is now active.',C['focus']:'Resume at CLEANUP-WBS-22-06 for Wave 3.',C['open']:'Waves 3-5 continue sequentially. Stop before Wave 6.',C['decisions']:'Bulk inside each wave only.',C['next']:'Prepare Wave 3 batch.',C['resume']:'Resume at WBS22 Wave 3; verify active task CLEANUP-WBS-22-06.',C['at']:ts,C['created']:ts,C['updated']:ts,C['trigger']:'milestone',C['github']:'airtable_only',C['status']:'active',C['retention']:'operational'}}]); actions.append('checkpoint')
 ...
 errors=[]
 if after['wbs04']['fields'].get('state')!='complete': errors.append('wbs04')
 if after['wbs05']['fields'].get('state')!='complete': errors.append('wbs05')
 if after['wbs06']['fields'].get('state')!='active': errors.append('wbs06')
 if after['plan']['fields'].get('active_task_id')!='CLEANUP-WBS-22-06': errors.append('plan')
