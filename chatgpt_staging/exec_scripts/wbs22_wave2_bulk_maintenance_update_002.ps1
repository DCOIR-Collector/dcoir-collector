...
$outDir = Join-Path $downloads 'wbs22_wave2_bulk_maintenance_update_002'
...
try {
    ...
    function Get-TableByName {
        ...
        $matches = @($Schema.tables | Where-Object { $_.name -eq $Name })
    ...
    function Get-FieldByName {
        ...
        $matches = @($Table.fields | Where-Object { $_.name -eq $Name })
        if ($matches.Count -lt 1) { return $null }
        return $matches[0]
    }
    ...
    function Invoke-AirtablePatchBatch {
        param([string]$BaseId, [string]$TableId, [hashtable]$Headers, $Records)
        $encodedTable = [System.Uri]::EscapeDataString($TableId)
        $uri = "https://api.airtable.com/v0/$BaseId/$encodedTable"
        $body = @{ records = @($Records); typecast = $false } | ConvertTo-Json -Depth 30
    ...
    $policyOut = [ordered]@{
        schema = 'dcoir.wbs22.wave2.bulk_policy.v3'
    ...
    foreach ($tableName in @($policyByTable.Keys)) {
        ...
        if ($null -eq $statusField) { $statusField = Get-FieldByName -Table $table -Name 'Status' }
        ...
        foreach ($record in $records) {
            ...
            $statusName = if ($null -ne $statusField) { [string]$statusField.name } else { 'status' }
            $currentStatus = Get-RecordFieldValue -Record $record -FieldName $statusName
            $primary = ''
            if ($table.primaryFieldId) {
                $primaryFieldMatches = @($table.fields | Where-Object { $_.id -eq $table.primaryFieldId })
                if ($primaryFieldMatches.Count -gt 0) { $primary = [string](Get-RecordFieldValue -Record $record -FieldName ([string]$primaryFieldMatches[0].name)) }
            }
            ...
            if (Is-BlankValue $currentRetention) {
                ...
                if ($null -eq $retentionField) { $skipReasons.Add('missing_retention_class_field_in_schema') | Out-Null }
            ...
            if (Is-BlankValue $currentReview) {
                ...
                if ($null -eq $reviewField) { $skipReasons.Add('missing_review_after_field_in_schema') | Out-Null }
                else { $fields['review_after'] = $desiredReview; $changes.Add("review_after=$desiredReview") | Out-Null }
            }

            if ($policy.Contains('status') -and ($null -ne $statusField) -and (Is-BlankValue $currentStatus)) {
    ...
    foreach ($tableName in @($updatesByTable.Keys | Sort-Object)) {
        ...
        foreach ($u in @($updatesByTable[$tableName].ToArray())) {
            ...
            $verified = $true
            $actual = [ordered]@{}
            foreach ($entry in $u.fields.GetEnumerator()) {
                $actualValue = Get-RecordFieldValue -Record $record -FieldName ([string]$entry.Key)
                $actual[[string]$entry.Key] = $actualValue
                if ([string]$actualValue -ne [string]$entry.Value) { $verified = $false }
            }
    ...
    $summary = [ordered]@{
        schema = 'dcoir.wbs22.wave2.bulk_update_summary.v3'
        generated_at_utc = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
