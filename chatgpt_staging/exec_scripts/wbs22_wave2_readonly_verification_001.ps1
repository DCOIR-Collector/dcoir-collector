...
$outDir = Join-Path $downloads 'wbs22_wave2_readonly_verification_001'
...
try {
    ...
    foreach ($policy in $tablePolicies) {
        ...
        foreach ($record in $records) {
            ...
            if ($hasBlank) {
                $remainingRows.Add([pscustomobject]@{ table_name=$policy.name; table_id=[string]$table.id; record_id=$rid; retention_class=$ret; review_after=$rev; status=$status; expected_retention=$policy.retention; expected_review_after=$policy.review_after; expected_status=$policy.status }) | Out-Null
            }
        }
        ...
        foreach ($r in @($records | Select-Object -First 5)) {
            $sampleRows.Add([pscustomobject]@{ table_name=$policy.name; record_id=[string]$r.id; retention_class=(Norm (GetField $r 'retention_class')); review_after=(Norm (GetField $r 'review_after')); status=(Norm (GetField $r 'status')); Status=(Norm (GetField $r 'Status')) }) | Out-Null
        }
    }

    $summary = [ordered]@{
        schema = 'dcoir.wbs22.wave2.readonly_verification.v1'
