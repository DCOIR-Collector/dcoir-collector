...
try {
    ...
    $candidatePath = Join-Path $repoRoot 'chatgpt_staging\status_reports\chatgpt-exec\exec-20260508-wbs22-wave2-discovery-exporter-007\artifact_readback\downloads\wbs22_wave2_discovery_from_exporter_005\wave2_deferred_candidates.json'
    if (-not (Test-Path -LiteralPath $candidatePath -PathType Leaf)) { throw "Missing deferred candidate input: $candidatePath" }

    $candidateText = Get-Content -LiteralPath $candidatePath -Raw -Encoding UTF8
    $candidates = @(ConvertFrom-Json -InputObject $candidateText)
    ...
    $policyByTable = [ordered]@{
        ...
        'Repo Surface Registry' = [ordered]@{ retention_class = 'review'; review_after = $waveReviewShort; basis = 'Repo Surface Registry rows flagged by reconnaissance need review/retention triage, not delete/archive.' }
    ...
    $tablesToConsider = @($candidates | ForEach-Object { [string]$_.table_name } | Sort-Object -Unique)

    $policyOut = [ordered]@{
        schema = 'dcoir.wbs22.wave2.bulk_policy.v1'
        ...
        scope = 'Wave 2 maintenance fields only: retention_class, review_after, and blank status where explicit policy exists.'
    ...
    foreach ($tableName in $tablesToConsider) {
        $policy = $policyByTable[$tableName]
        $table = Get-TableByName -Schema $schema -Name $tableName
        if ($null -eq $table -or $table.Count -eq 0) {
            foreach ($candidate in @($candidates | Where-Object { $_.table_name -eq $tableName })) {
                $skipped.Add([pscustomobject][ordered]@{ table_name=$tableName; record_id=[string]$candidate.record_id; reason='table_not_found_in_live_schema' }) | Out-Null
            }
            continue
        }
        if ($null -eq $policy) {
            foreach ($candidate in @($candidates | Where-Object { $_.table_name -eq $tableName })) {
                $skipped.Add([pscustomobject][ordered]@{ table_name=$tableName; table_id=[string]$table.id; record_id=[string]$candidate.record_id; reason='no_deterministic_wave2_policy_for_table'; candidate_reasons=@($candidate.reasons) }) | Out-Null
            }
            continue
        }
        ...
        Write-Host "Fetching live records for table $tableName ($($table.id))"
        ...
        $recordCacheByTable[$tableName] = $recordMap

        foreach ($candidate in @($candidates | Where-Object { $_.table_name -eq $tableName })) {
            $recordId = [string]$candidate.record_id
            if (-not $recordMap.ContainsKey($recordId)) {
                $skipped.Add([pscustomobject][ordered]@{ table_name=$tableName; table_id=[string]$table.id; record_id=$recordId; reason='candidate_record_not_found_live'; candidate_reasons=@($candidate.reasons) }) | Out-Null
                continue
            }
            ...
            $beforeRows.Add([pscustomobject][ordered]@{
                ...
                record_id = $recordId
                primary_value = [string]$candidate.primary_value
            ...
            if (@($candidate.reasons) -match 'missing_retention_class' -and Is-BlankValue $currentRetention) {
                $desiredRetention = [string]$policy.retention_class
                if ($null -eq $retentionField -or $retentionField.Count -eq 0) {
                    $skipReasons.Add('missing_retention_class_field_in_schema') | Out-Null
                } elseif ($retentionField.type -eq 'singleSelect' -and ($retentionChoices -notcontains $desiredRetention)) {
                    $skipReasons.Add("retention_choice_not_allowed:$desiredRetention") | Out-Null
                } else {
                    $fields['retention_class'] = $desiredRetention
                    $changes.Add("retention_class=$desiredRetention") | Out-Null
                }
            }

            if (@($candidate.reasons) -match 'missing_review_after' -and Is-BlankValue $currentReview) {
                $desiredReview = [string]$policy.review_after
                if ($null -eq $reviewField -or $reviewField.Count -eq 0) {
                    $skipReasons.Add('missing_review_after_field_in_schema') | Out-Null
                } else {
                    $fields['review_after'] = $desiredReview
                    $changes.Add("review_after=$desiredReview") | Out-Null
                }
            }
            ...
            if ($policy.Contains('status') -and ($null -ne $statusField -and $statusField.Count -gt 0) -and (Is-BlankValue $currentStatus)) {
                ...
                if ($statusField.type -eq 'singleSelect' -and ($statusChoices -notcontains $desiredStatus)) {
                    $skipReasons.Add("status_choice_not_allowed:$desiredStatus") | Out-Null
                } else {
                    $fields[$statusName] = $desiredStatus
                    $changes.Add("$statusName=$desiredStatus") | Out-Null
                }
            }

            if ($fields.Count -eq 0) {
                $reason = if ($skipReasons.Count -gt 0) { ($skipReasons.ToArray() -join ';') } else { 'no_blank_policy_field_to_update' }
                $skipped.Add([pscustomobject][ordered]@{ table_name=$tableName; table_id=[string]$table.id; record_id=$recordId; primary_value=[string]$candidate.primary_value; reason=$reason; candidate_reasons=@($candidate.reasons) }) | Out-Null
                continue
            }
    ...
    foreach ($tableName in @($updatesByTable.Keys | Sort-Object)) {
        ...
        foreach ($u in @($updatesByTable[$tableName].ToArray())) {
            ...
            if (-not $recordMap.ContainsKey($recordId)) {
                $verificationFailures.Add([pscustomobject][ordered]@{ table_name=$tableName; record_id=$recordId; reason='updated_record_missing_on_readback' }) | Out-Null
                continue
            }
            $record = $recordMap[$recordId]
            $verified = $true
            ...
            if (-not $verified) {
                $verificationFailures.Add([pscustomobject][ordered]@{ table_name=$tableName; record_id=$recordId; expected=$u.fields; actual=$actual; reason='field_value_mismatch' }) | Out-Null
            }
    ...
    $summary = [ordered]@{
        schema = 'dcoir.wbs22.wave2.bulk_update_summary.v1'
        generated_at_utc = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
    ...
    $lines.Add('') | Out-Null
    $lines.Add("- candidate_count: $($summary.candidate_count)") | Out-Null
    ...
    $lines.Add('') | Out-Null
    $lines.Add('Applied only Wave 2 maintenance fields: retention_class, review_after, and blank status where explicit/default table policy existed. No deletes, Delete Queue processing, schema changes, duplicate/merge work, GitHub/source/skill/workflow changes, or scaffold disposition.') | Out-Null
    $lines.Add('') | Out-Null
    $lines.Add('## Artifacts') | Out-Null
    $lines.Add('') | Out-Null
    foreach ($name in @('wave2_bulk_policy.json','wave2_bulk_before.json','wave2_bulk_update_plan.json','wave2_bulk_update_results.json','wave2_bulk_after_readback.json','wave2_bulk_verification_failures.json','wave2_bulk_skipped.json','wave2_bulk_summary.json','terminal_transcript.txt')) {
