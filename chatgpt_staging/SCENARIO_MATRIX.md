...
| ChatGPT initiates manual artifact readback | Run `manual-github-artifact-readback` with the source run id plus a bounded artifact name or id, then read the staged output under `chatgpt_staging/out/<request_id>/`. | Download the chosen artifact, extract it into `chatgpt_staging/out/<request_id>/`, and write a committed workflow report. | Work Item remains active/waiting until extracted output is read and cleanup is recorded. |
...
| Manual artifact readback succeeds | Read `artifact_manifest.md` and the staged files under `chatgpt_staging/out/<request_id>/`, then clean them when safe. | Preserve `out/<request_id>` and `status_reports/manual-github-artifact-readback/<request_id>/` until cleanup marker appears. | Record extracted-artifact readback before cleanup. |
