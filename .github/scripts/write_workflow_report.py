...
def make_workflow_report(args: argparse.Namespace) -> int:
    ...
    result = "success" if conclusion == "success" else "failure" if conclusion in failure_conclusions else "neutral" if conclusion in neutral_conclusions else conclusion
    action_hint = (
        ...
        if result == "success"
        else "No repair is implied by this neutral workflow conclusion. Confirm the source workflow was intentionally skipped or neutral, then use workflow_run_url only if the skip reason is unclear. Do not use this completed-run summary for live polling; active ChatGPT-staged jobs must be monitored through their live-heartbeat request_id report."
        if result == "neutral"
        else "Use the embedded bounded log excerpt first. If it is not enough, open workflow_run_url for full GitHub Actions logs/artifacts before asking the operator for screenshots or pasted logs. Do not use this completed-run summary for live polling; active ChatGPT-staged jobs must be monitored through their live-heartbeat request_id report."
    )
    ...
    if result == "failure":
        ...
        lines.append("The source workflow ended with a neutral/skipped conclusion; no failure log excerpt is embedded by default.")
    else:
