...
The attachment map, the maintained `knowledge/*.md` set, and the manifest `knowledge_attachment_sources` inventory must stay synchronized. If the shared attachment set changes, update those surfaces in the same bounded change set so manual build, validate-on-push, and compile all continue to agree about what the bundle is supposed to ship.
