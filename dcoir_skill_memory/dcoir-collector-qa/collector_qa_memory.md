...
exported_at_utc: 2026-04-18T08:30:00Z
authority_basis:
...
## Current focus
Track the bounded issue #42 source patch after in-session GitHub promotion. Keep runtime validation pending and keep issue #42 open until one Collect run proves overview-path stdout, manifest inclusion, bundle inclusion, and smaller-surface guidance ordering.
...
- **Collector smaller analyst-first overview surface (#42)** (status: source-patched-pending-runtime-validation)
  - details: Added a bounded source patch to emit DCOIR_ANALYST_OVERVIEW and wire ANALYST_OVERVIEW_PATH into collect-run state, manifest, bundle, stdout, and Gemini upload guidance while preserving DCOIR_BASELINE for deeper local review.
  - next_action: Run one Collect flow after promotion and verify ANALYST_OVERVIEW_PATH stdout emission, overview file creation, manifest inclusion, bundle inclusion, and that the guidance prefers the overview before the monolithic baseline report. Keep issue #42 open until that evidence exists.
...
- Issue #42 now has a bounded source patch ready for promotion in the maintained collector line: the new analyst overview artifact is wired into collect-run state, manifest, bundle, stdout, and Gemini upload guidance without removing the deeper baseline report.

## Next actions
- Run one Collect flow after the repo write and verify ANALYST_OVERVIEW_PATH, overview-file creation, bundle inclusion, manifest inclusion, and guidance ordering.
- Keep issue #42 open until runtime validation proves the smaller-surface behavior on an actual collect run.
...
## Provenance notes
- Updated after preparing and promoting the bounded issue #42 source patch in-chat.
- Airtable Validation Test Cases row COL-OVERVIEW-001 was created with status Partial because live Collect-run evidence is still pending.
