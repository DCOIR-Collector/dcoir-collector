...
$outDir = Join-Path $downloads ('dcoir_wbs09_relative_date_popup_probe_' + $timestamp)
New-Item -ItemType Directory -Path $outDir -Force | Out-Null
$logPath = Join-Path $outDir 'relative_date_popup_probe_launcher.log'
...
Write-ToolLog 'Starting DCOIR WBS09 relative-date popup probe launcher.'
...
Write-ToolLog ('Field name: ' + $FieldName)
...
$nodeScript = Join-Path $toolRoot 'scripts\airtable_wbs09_relative_date_popup_probe.mjs'
...
try {
    ...
    if ($exit -ne 0) { throw ('WBS09 relative-date popup probe failed with exit code ' + $exit) }
    Write-ToolLog 'Relative-date popup probe launcher completed successfully.'
