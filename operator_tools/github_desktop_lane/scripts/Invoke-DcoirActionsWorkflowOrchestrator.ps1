...
Safety defaults:
  ...
  - Live dispatch requires -ConfirmDispatch unless manifest require_dispatch_confirmation is false.
  - Live dispatch defaults to one workflow run only. More than one live dispatch requires allow_multiple_live_dispatches=true.
...
function Save-DcoirJson {
    ...
    Write-DcoirUtf8Text -Path $Path -Text ($Object | ConvertTo-Json -Depth 30)
}
...
function Write-Phase {
    ...
    $Script:Phase = $Name
    Write-Status ("PHASE {0}: {1}" -f $Name, $Message)
}
...
function Save-DcoirRunEvidence {
    ...
    if ($wantLogs) {
        try { Write-DcoirUtf8Text -Path (Join-Path $runDir 'run.log.txt') -Text (Invoke-DcoirGhText -GhArgs @('run','view', [string]$Id, '-R', $script:Repo, '--log') -DebugName "log_$Id") }
...
function New-DcoirRunRecord {
    ...
    return [ordered]@{
        ...
        created_at = $null; updated_at = $null; error = $null; dispatch_started_at = $null
    }
}

function Assert-DcoirLiveDispatchAllowed {
    param([System.Collections.ArrayList]$Records)
    if ($script:ManifestDryRun) { return }
    $plannedCount = @($Records | Where-Object { $_.state -eq 'planned' }).Count
    if ($plannedCount -gt $script:MaxDispatchCount) {
        $script:DispatchesBlocked++
        throw ("Live dispatch blocked: manifest has {0} planned runs, max_dispatch_count={1}. Reduce runs[] or set a higher max_dispatch_count intentionally." -f $plannedCount, $script:MaxDispatchCount)
    }
    if ($plannedCount -gt 1 -and -not $script:AllowMultipleLiveDispatches) {
        $script:DispatchesBlocked++
        throw ("Live dispatch blocked: manifest has {0} runs. Set allow_multiple_live_dispatches=true only after reviewing the plan." -f $plannedCount)
    }
}
...
function Start-DcoirWorkflowRun {
    ...
    Write-Phase -Name 'dispatch' -Message ("Dispatching workflow '{0}' for run label '{1}' on ref '{2}' with inputs: {3}" -f $workflowFile, $Record.label, $refName, (Format-DcoirInputMap -Map $Record.inputs))
    ...
    if ($inputs) { foreach ($key in $inputs.Keys) { $args += @('-f', ("{0}={1}" -f $key, [string]$inputs[$key])) } }
    ...
    $script:DispatchesStarted++
    Save-DcoirJson -Path (Join-Path $script:RunOutputDir 'dispatch_ledger.json') -Object @{ dispatches_started = $script:DispatchesStarted; max_dispatch_count = $script:MaxDispatchCount; records = @($script:Records) }
    for ($i = 1; $i -le $script:DispatchPollAttempts; $i++) {
        ...
        Write-Status ("Waiting for dispatched run to appear for {0} ({1}/{2}); no additional dispatch will occur during this wait." -f $Record.label, $i, $script:DispatchPollAttempts)
    }
...
function Wait-DcoirRunSet {
    ...
    Write-Phase -Name 'monitor' -Message ("Monitoring workflow run set with {0} record(s)." -f $Records.Count)
    ...
    while ($true) {
        ...
        foreach ($r in @($Records | Where-Object { $_.state -eq 'active' })) {
            Write-Status ("Monitoring workflow run {0} ({1}) status={2} conclusion={3}" -f $r.run_id, $r.label, $r.status, $r.conclusion)
...
function Write-DcoirExecutionPlan {
    param([System.Collections.ArrayList]$Records)
    Write-Phase -Name 'plan' -Message 'Execution plan resolved.'
    Write-Status ("Mode={0}; Repo={1}; Ref={2}; DryRun={3}; MaxParallel={4}; MaxDispatchCount={5}; AllowMultipleLiveDispatches={6}" -f $Mode, $script:Repo, $script:DefaultRef, $script:ManifestDryRun, $script:MaxParallel, $script:MaxDispatchCount, $script:AllowMultipleLiveDispatches)
    Write-Status ("Run set id={0}; output_dir={1}" -f $script:RunSetId, $script:RunOutputDir)
    $i = 0
    foreach ($r in $Records) {
        $i++
        Write-Status ("Plan[{0}]: label={1}; workflow={2}; ref={3}; state={4}; inputs={5}" -f $i, $r.label, $r.workflow, $r.ref, $r.state, (Format-DcoirInputMap -Map $r.inputs))
    }
}
...
if ([string]::IsNullOrWhiteSpace($ManifestJson) -and $Mode -eq 'manifest') { $ManifestJson = Join-Path $script:SystemRepoRoot 'operator_tools\github_desktop_lane\manifests\actions_workflow_orchestrator.dispatch.sample.json' }
...
$script:DefaultRef = [string](Get-DcoirConfigValue -Map $manifest -Name 'default_ref' -Default $Ref)
...
$script:ManifestDryRun = [bool](Get-DcoirConfigValue -Map $manifest -Name 'dry_run' -Default $true)
if ($DryRun) { $script:ManifestDryRun = $true }
$script:AllowMultipleLiveDispatches = [bool](Get-DcoirConfigValue -Map $manifest -Name 'allow_multiple_live_dispatches' -Default $false)
$script:MaxDispatchCount = [int](Get-DcoirConfigValue -Map $manifest -Name 'max_dispatch_count' -Default 1)
if ($script:ManifestDryRun) { $script:MaxDispatchCount = [int](Get-DcoirConfigValue -Map $manifest -Name 'max_dispatch_count' -Default 9999) }
if ($script:MaxDispatchCount -lt 1) { $script:MaxDispatchCount = 1 }
...
$script:RunSetId = [string](Get-DcoirConfigValue -Map $manifest -Name 'run_set_id' -Default ("dcoir_actions_" + (Get-Date -Format 'yyyyMMdd_HHmmss')))
$stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$script:RunOutputDir = Join-Path $outputFolder ("{0}_{1}" -f $stamp, ($script:RunSetId -replace '[^A-Za-z0-9_.-]', '_'))
...
$zipName = [string](Get-DcoirConfigValue -Map $outputCfg -Name 'zip_name' -Default ($script:RunSetId + '.chatgpt.zip'))
...
if ($Mode -eq 'dispatch') {
    ...
    foreach ($r in @($runs)) {
        ...
        $rref = [string](Get-DcoirConfigValue -Map $r -Name 'ref' -Default $script:DefaultRef)
    ...
    Write-DcoirExecutionPlan -Records $records
    Save-DcoirJson -Path (Join-Path $script:RunOutputDir 'dispatch_plan.json') -Object @($records)
    if ($script:ManifestDryRun) {
        Write-Phase -Name 'dry_run' -Message 'Dry run requested. No workflows dispatched.'
    ...
    Add-DcoirWatchRecords -Records $records -Workflows $wfs -Ids $ids -LimitCount $lim -Branch $script:DefaultRef
    Write-DcoirExecutionPlan -Records $records
    ...
    Add-DcoirWatchRecords -Records $records -Workflows @() -Ids $ids -LimitCount $Limit -Branch $script:DefaultRef
    Write-DcoirExecutionPlan -Records $records
...
$summary = [ordered]@{
    ...
    mode = $Mode; repo = $script:Repo; default_ref = $script:DefaultRef; run_set_id = $script:RunSetId; dry_run = $script:ManifestDryRun; max_parallel = $script:MaxParallel
    allow_multiple_live_dispatches = $script:AllowMultipleLiveDispatches; max_dispatch_count = $script:MaxDispatchCount; dispatches_started = $script:DispatchesStarted; dispatches_succeeded = $script:DispatchesSucceeded; dispatches_blocked = $script:DispatchesBlocked
...
$md += "- Created: $($summary.created_at)"; $md += "- Mode: $Mode"; $md += "- Repo: $script:Repo"; $md += "- Run set: $script:RunSetId"; $md += "- Dry run: $script:ManifestDryRun"; $md += "- Max parallel: $script:MaxParallel"; $md += "- Max dispatch count: $script:MaxDispatchCount"; $md += "- Dispatches started: $script:DispatchesStarted"; $md += "- Machine DCOIR_REPO_ROOT: $script:SystemRepoRoot"; $md += ''
$md += '| Label | Workflow | Run ID | State | Status | Conclusion | URL |'; $md += '|---|---|---:|---|---|---|---|'
foreach ($rec in $records) { $md += "| $($rec.label) | $($rec.workflow) | $($rec.run_id) | $($rec.state) | $($rec.status) | $($rec.conclusion) | $($rec.url) |" }
Write-DcoirUtf8Text -Path (Join-Path $script:RunOutputDir 'orchestrator_summary.md') -Text ($md -join "`n")

Write-Phase -Name 'package' -Message ("Session summary before ZIP: records={0}; dispatches_started={1}; failures={2}; output={3}" -f $records.Count, $script:DispatchesStarted, $failed.Count, $script:RunOutputDir)
if ($createZip) {
...
Write-Phase -Name 'complete' -Message "Complete. Output: $script:RunOutputDir"
if ($summary.zip_path) { Write-Host "UPLOAD_ZIP=$($summary.zip_path)" }
if ($failed.Count -gt 0 -or $script:DispatchesBlocked -gt 0) { exit 2 }
