...
param(
    ...
    [string]$TargetKey = 'Operator Tools Registry::WBS09 - Validation Due',
    [string]$ManifestPath,
    [string]$BaseUrl,
...
if ($ConfirmToken -ne 'APPLY_WBS09_VALIDATION_DUE_VIEW') {
    throw 'This mutation helper requires -ConfirmToken APPLY_WBS09_VALIDATION_DUE_VIEW.'
}

if ($TargetKey -ne 'Operator Tools Registry::WBS09 - Validation Due') {
    throw 'This guarded helper only supports TargetKey "Operator Tools Registry::WBS09 - Validation Due".'
}
...
$outDir = Join-Path $downloads ('dcoir_wbs09_apply_validation_due_view_' + $timestamp)
New-Item -ItemType Directory -Path $outDir -Force | Out-Null
$logPath = Join-Path $outDir 'apply_validation_due_view_launcher.log'
...
Write-ToolLog 'Starting DCOIR WBS09 apply validation-due view launcher.'
...
$nodeScript = Join-Path $toolRoot 'scripts\airtable_wbs09_apply_validation_due_view.mjs'
...
try {
    ...
    if ($exit -ne 0) { throw ('WBS09 apply validation-due view failed with exit code ' + $exit) }
    Write-ToolLog 'Apply validation-due view launcher completed successfully.'
