...
try {
    ...
    if (-not $NoGitCleanCheck) {
        ...
        if ($statusLines.Count -gt 0) {
            ...
            Write-Log "Git working tree appears clean."
        }
    }
    ...
    foreach ($file in $allFiles) {
        $full = $file.FullName
        if (-not (Test-UnderPath -Path $full -Root $RepoRoot)) {
            Add-Skip -RelativePath $full -Reason "outside_repo_root" -Size $file.Length
            continue
        }
        ...
        if ($excludedDir) {
            Add-Skip -RelativePath $rel -Reason "excluded_directory:$excludedDir" -Size $file.Length
            continue
        }

        $ext = $file.Extension.ToLowerInvariant()
        if ($ExcludeExtensions -contains $ext) {
            Add-Skip -RelativePath $rel -Reason "excluded_extension:$ext" -Size $file.Length
            continue
        }
        if ($file.Length -gt $MaxFileBytes) {
            Add-Skip -RelativePath $rel -Reason "over_max_file_bytes" -Size $file.Length
            continue
        }
        if (Test-LikelyBinary -Path $full) {
            Add-Skip -RelativePath $rel -Reason "binary_sniff" -Size $file.Length
            continue
        }
